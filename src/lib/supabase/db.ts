...
export async function fetchHabits(): Promise<Habit[]> {
  ...
  if (error) { console.error('fetchHabits:', error.message); return []; }
...
export async function fetchBooks(): Promise<Book[]> {
  ...
  if (error) { console.error('fetchBooks:', error.message); return []; }
...
export async function fetchProductivityHistory(): Promise<Record<string, { completed: number; total: number }>> {
  ...
  if (error) { console.error('fetchProductivityHistory:', error.message); return {}; }
...
export async function purgeAllData(): Promise<void> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;
  for (const table of ['tasks', 'habits', 'goals', 'books', 'productivity_history']) {
