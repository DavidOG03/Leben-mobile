...
export function EfficiencyScore() {
  ...
  const analytics = useMemo(() => {
    ...
    const getDayScore = (dateStr: string): number | null => {
      ...
      const existingHabits = habits.filter(
        (h) => !h.createdAt || h.createdAt.split("T")[0] <= dateStr,
      );
      const habitsCompleted = existingHabits.filter((h) =>
      ...
      let wSum = 0,
        wTotal = 0;
      if (dayScheduled > 0) {
        wSum += (dayCompleted / dayScheduled) * 0.4;
        wTotal += 0.4;
      }
      if (existingHabits.length > 0) {
        wSum += (habitsCompleted / existingHabits.length) * 0.6;
        wTotal += 0.6;
      }
    ...
    for (const dateStr of weekDates) {
      ...
      const dayHabitsCompleted = existingHabits.filter((h) =>
        h.completedDates?.includes(dateStr),
      ).length;
      ...
      if (dayScheduled > 0 || existingHabits.length > 0 || dayMilestones > 0)
        weeklyActiveDays++;
    }
    ...
    const habitRate =
      totalPossibleHabitDays > 0
        ? totalCompletedHabitDays / totalPossibleHabitDays
        : 0;
    const goalRate =
    ...
    if (totalScheduledTasks > 0) {
      wSum += taskRate * weights.task;
      wTotal += weights.task;
    }
    if (totalPossibleHabitDays > 0) {
      wSum += habitRate * weights.habit;
      wTotal += weights.habit;
    }
    if (totalMilestones > 0) {
      wSum += goalRate * weights.goal;
      wTotal += weights.goal;
    }
    if (totalBooks > 0) {
      wSum += bookRate * weights.book;
      wTotal += weights.book;
    }
  ...
  const strokeBorder = colorScheme === "dark" ? "#1e1e1e" : "#99999975";
