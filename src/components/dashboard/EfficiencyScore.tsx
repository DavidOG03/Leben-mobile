...
import Animated, { useAnimatedProps, withTiming, Easing, useSharedValue } from 'react-native-reanimated';
...
export function EfficiencyScore() {
  ...
  const analytics = useMemo(() => {
    ...
    const allActivityDates = [
      ...
      ...goals.flatMap((g) =>
        g.milestones
          .filter((m) => m.completed && m.id) // web uses m.done and m.completedAt
          .map((m) => todayIso) // approximated since mobile goal shape is slightly different
      ),
    ];
    ...
    const daysSinceFirstActivity = firstActivityDate
      ? Math.floor((new Date(todayIso).getTime() - new Date(firstActivityDate).getTime()) / msPerDay)
      : 0;
    ...
    for (const dateStr of weekDates) {
      const dayTasks = tasks.filter((t) => t.date === dateStr || t.completedAt?.split('T')[0] === dateStr);
      const dayCompletedTasks = dayTasks.filter((t) => t.completed && t.completedAt?.split('T')[0] === dateStr).length;
      ...
      if (dayTasks.length > 0 || dayHabits > 0) {
        weeklyActiveDays += 1;
      }
    }
    ...
    const hasEnoughData = firstActivityDate !== undefined && daysSinceFirstActivity >= 6 && weeklyActiveDays > 0;
  ...
  return (
    <Card className="min-h-[260px] items-center justify-center p-7" style={{ backgroundColor: '#121212', borderColor: '#1e1e1e' }}>
        ...
        <View className="items-center justify-center w-full">
          <View className="relative items-center justify-center mb-5">
            <Svg width="140" height="140" viewBox="0 0 140 140">
              ...
              <Circle 
                cx="70" cy="70" r="54" fill="none" stroke="#252525" strokeWidth="6" 
                strokeLinecap="round" strokeDasharray="8 6" origin="70, 70" rotation="-90" 
              />
            </Svg>
          ...
          >
            <Text className="text-leben-accent font-semibold text-xs">Sign In</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="items-center justify-center w-full">
          <View className="relative items-center justify-center mb-5">
            <Svg width="140" height="140" viewBox="0 0 140 140">
              ...
              <Circle 
                cx="70" cy="70" r="54" fill="none" stroke="#252525" strokeWidth="8" 
                strokeLinecap="round" strokeDasharray="12 8" origin="70, 70" rotation="-90" 
              />
            </Svg>
            <View className="absolute items-center">
              <Text className="text-[#2e2e2e] text-3xl font-bold leading-none">—</Text>
              <Text className="text-[#2e2e2e] text-[9px] uppercase tracking-widest mt-1">No data</Text>
            </View>
          </View>
        ...
        <View className="items-center justify-center w-full">
          <View className="relative items-center justify-center mb-5">
            <Svg width="140" height="140" viewBox="0 0 140 140">
              <Circle cx="70" cy="70" r="54" fill="none" stroke="#1a1a1a" strokeWidth="8" />
              <AnimatedCircle 
                cx="70" cy="70" r="54" fill="none" stroke="#7c6af0" strokeWidth="8" 
